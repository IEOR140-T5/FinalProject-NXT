...
public class Locator {
	...
	static final float differenceFactor = 5.0f; // it's how off it's from the center of the robot when rotating
...
/**		if (_pose.getHeading() == 90){
	...
	public Pose fixPosition(float[] bearings, float echoDistance) {
 	...
 	public Pose fixPosition(float[] bearings, float echoDistance) {
		...
		float Ytotal = hallWidth;
		float x = 0;
		float y1 = echoDistance;
		float y2 = Ytotal - y1;
		float theta = normalize(bearings[0] - bearings[1]);

		float theta2 = (float) Math.abs(theta);

		System.out.println(theta);

		// Quadratic Formula Variables
		float a = (float) Math.tan(Math.toRadians(theta2));
		float b = -Ytotal;
		float c = (float) -Math.tan(Math.toRadians(theta2)) * y1 * y2;
