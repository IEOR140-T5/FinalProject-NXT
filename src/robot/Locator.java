...
public class Locator {
	...
	static final float differenceFactor = 5.5f; // it's how off it's from the
												// center of the robot when
												// rotating
	 ...
	 * 
	 * @param s
	 *            - the scanner to be initialized and set
	 */
	...
	public float[] scanBeacons() {
		...
		float angleToNorthWall = normalize(_pose
				.relativeBearing(new Point(x, 0))) + angleError;
		float angleToSouthWall = normalize(_pose.relativeBearing(new Point(x,
				hallWidth))) + angleError;
		...
		if (isFirstQuadrant) {
			if (isYNearZeroBeacon) {
				scanner.scanLights((int) angleToNorthWall,
						(int) angleToNorthWall - 180);
				System.out.println("1st Quad");
			} else {
				scanner.scanLights((int) angleToSouthWall,
						(int) angleToSouthWall + 180);
	...
	/**
	 * Locate the current coordinate of the robot It will find an appropriate
	 * heading to scan 180 degree which contains 2 lights
	 * 
	...
	public float[] locate() {
		...
		if (isYNearZeroBeacon) {
			...
			distanceToWall = (int) hallWidth
					- scanner.getDistanceToWall(angleToSouthWall);
			beaconsBearings = scanBeacons();
		...
		System.out.println(distanceToWall + "," + beaconsBearings[0] + ","
				+ beaconsBearings[1]);
		...
		_pose.setLocation(_pose.pointAt(differenceFactor,
	 ...
	 * 
	 * @param bearings
	 *            - the bearings of the lights
	 * @param echoDistance
	 *            - how far is it away from the closest wall
	 * @return a new Pose
	 */
	/*
	 * public Pose fixPosition(float[] bearings, float echoDistance) { float x =
	 * 0; float y1 = echoDistance; float y2 = beaconY - y1; float
	 * differenceInDeg = normalize(bearings[0] - bearings[1]); boolean
	 * isDifferenceInDegNegative = normalize(bearings[0] - bearings[1]) < 0;
	 * float differenceInRad = (float)Math.toRadians(Math.abs(differenceInDeg));
	 * 
	 * float a = (float) Math.tan(differenceInRad); float b = -beaconY; float c
	 * = (float) -Math.tan(differenceInRad * y1 * y2); float delta = (float)
	 * (Math.pow(b, 2) - 4 * a * c); float x1 = (float) (-b + Math.sqrt(delta))
	 * / (2 * a); float x2 = (float) (-b - Math.sqrt(delta)) / (2 * a);
	 * 
	 * System.out.println(x1 + "-" + x2); x = (Math.abs(differenceInDeg) < 90) ?
	 * x1 : x2; x = (isDifferenceInDegNegative) ? x*-1 : x*1;
	 * 
	 * 
	 * float newHeading = normalize(_pose.angleTo(beacon[0]) - bearings[0]);
	 * _pose.setHeading(newHeading);
	 * 
	 * // hardcode heading /* if (_pose.getHeading() == 0){
	 ...
	 * } 0----
	 * 
	 * _pose.setLocation( (float) (x - Math.cos(Math.toRadians(newHeading))) -
	 * 6, echoDistance);
	 * 
	 * return _pose; }
	 */
